...
export function ActivityItem({ item, currentUser }) {
  ...
  if (item.kind === 'comment') {
    ...
    body = (
      <>
        <span className="text-secondary">{who} created this task</span>
        {item.title ? (
          <>
    ...
    body = (
      <>
        <span className="text-secondary">{who} changed status from </span>
        <StatusPhrase
    ...
    body = (
      <>
        <span className="text-secondary">{who} assigned to: </span>
        {toName && toName !== 'Empty' ? (
    ...
    body = (
      <>
        <span className="text-secondary">{who} set priority to </span>
    ...
    body = (
      <>
        <span className="text-secondary">{who} changed the task name from </span>
        <span className="text-secondary">{meta.from || 'Empty'}</span>
        <span className="text-secondary"> → </span>
        <span className="font-medium text-primary">{meta.to || 'Empty'}</span>
      </>
    )
  } else if (
    field === 'timeTracking' ||
    ...
    body = <span className="text-secondary">{text}</span>
  }

  return (
    <div className="group flex gap-2 px-1 py-1.5 text-[12.5px] leading-snug">
      <span className="mt-[7px] h-1 w-1 shrink-0 rounded-full bg-[#c7c7c7]" />
      <div className="min-w-0 flex-1">
        <div className="flex items-start gap-2">
          <p className="min-w-0 flex-1 text-secondary">{body}</p>
          <span className="shrink-0 pt-0.5 text-[11px] text-secondary">{when}</span>
        </div>
      </div>
    </div>
  )
